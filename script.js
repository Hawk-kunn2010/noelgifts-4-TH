...
function createGift() {
  ...
  gift.addEventListener('click', (e) => {
    e.stopPropagation(); // tránh click gây pháo hoa/thư ngoài ý muốn
  
    // (Tuỳ chọn) âm thanh mở quà nếu bạn có file:
    // const unwrapSound = new Audio('unwrap.mp3');
    // unwrapSound.play();
  
    // Không hiện thư nữa, chỉ remove hộp quà
...
// Lắng nghe sự kiện khi nhấn đóng thư
document.addEventListener('DOMContentLoaded', () => {
  const overlay = document.getElementById('letterOverlay');
  const closeBtn = document.getElementById('letterClose');
  const letterBox = document.getElementById('letterBox');

  // Khi nhấn nút đóng (✕), đóng thư
  if (closeBtn) closeBtn.addEventListener('click', (e) => {
    e.stopPropagation(); // Ngừng sự kiện nổi bọt
    closeLetter();  // Đóng thư
  });

  // Khi nhấn ra ngoài thư, đóng thư
  if (overlay) overlay.addEventListener('click', () => closeLetter());

  // Khi nhấn ESC để đóng thư
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') closeLetter();
  });

  // Khi click vào cây thông, mở thư
  const tree = document.querySelector('.tree');
  tree.addEventListener('click', (e) => {
    e.stopPropagation(); // Ngừng sự kiện nổi bọt
    openLetter();  // Mở thư khi bấm vào cây thông
  });
});